...
# Post work items
blackboard work create --id my-app-rss-retry \
  --title "Add retry logic to RSS fetcher" \
  --project my-app --priority P1

# An agent claims work
blackboard agent register --name "feature-builder" --project my-app
blackboard work claim my-app-rss-retry --session $SESSION_ID
...
# It reads a configurable checklist for what to check:
...
blackboard work create --id project-a-dashboard \
...
# project-a-dashboard   Health monitoring dashboard               available  P3        project-a
```
...
# PROJECT: My API Service (project-b)
# ACTIVE AGENTS (1):
...
```
 ...
 │  project-a-dashboard ······ available ···· P3                │
...
```bash
# Build the blackboard CLI
cd ivy-blackboard && bun build src/index.ts --compile --outfile dist/blackboard
...
blackboard project register --id my-app --name "My Application" --path ~/work/my-app
blackboard project register --id my-api --name "My API Service" --path ~/work/my-api
